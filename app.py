...
def generate_pdf(df, apartment):
    ...
    # Clean string matching to prevent empty dataframes due to trailing spaces
    df["Apartment Number"] = df["Apartment Number"].astype(str).str.strip().str.upper()
    target_apt = str(apartment).strip().upper()

    df = df[df["Apartment Number"] == target_apt]
...
if data_csvs:
    ...
    apartments = sorted(df["Apartment Number"].astype(str).str.strip().unique())
else:
...
if df is not None and apartment:
    if code_df is None:
        ...
        matched_row = code_df[
            code_df[apt_col].astype(str).str.strip().str.upper() == str(apartment).strip().upper()
        ]
